...
namespace WorkforceManager.Tests
{
    ...
    public class SearchMatcherTests
    {
        ...
        public void Matching_across_tens_of_thousands_of_rows_stays_fast()
        {
            ...
            // الهدف هنا إثبات عدم وجود انفجار في التعقيد (O(n²) أو أسوأ)،
            // مش SLA دقيق — رقم أكبر بكتير من المتوقع في الاستخدام الحقيقي
            // (سجل العمليات، أكبر فئة، محدود بـ400 يوم، مش 40 ألف صف على
            // طول) على صندوق بحث فيه debounce أصلًا 250ms. الحد هنا سخي
            // عن قصد عشان مايتكسرش بتفاوت سرعة أجهزة الاختبار/الحمل
            // المتوازي مع باقي الاختبارات، مع إنه لسه بيمسك أي تراجع حقيقي
            // في الأداء (رقم عشرات آلاف الـms لو الخوارزمية بقت O(n²) فعلًا)
