// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using WorkforceManager.Data;

#nullable disable

namespace WorkforceManager.Data.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260731124346_AddOpeningBalanceBatches")]
    partial class AddOpeningBalanceBatches
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder.HasAnnotation("ProductVersion", "8.0.8");

            modelBuilder.Entity("WorkforceManager.Core.Models.AppUser", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("DisplayName")
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.Property<string>("PasswordSalt")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("TEXT");

                    b.Property<string>("Username")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("Username")
                        .IsUnique();

                    b.ToTable("AppUsers");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Attendance", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<TimeSpan?>("CheckInTime")
                        .HasColumnType("TEXT");

                    b.Property<TimeSpan?>("CheckOutTime")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("Date")
                        .HasColumnType("TEXT");

                    b.Property<string>("Notes")
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.Property<int>("Status")
                        .HasColumnType("INTEGER");

                    b.Property<int>("WorkerId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("Date");

                    b.HasIndex("WorkerId", "Date")
                        .IsUnique();

                    b.ToTable("Attendances");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.DailyProduction", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("Date")
                        .HasColumnType("TEXT");

                    b.Property<string>("Notes")
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.Property<int>("PieceCount")
                        .HasColumnType("INTEGER");

                    b.Property<int>("PiecesPerWorkdayAtEntry")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("ProductionBatchId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("ProductionStageId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("WorkerId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("Date");

                    b.HasIndex("ProductionBatchId");

                    b.HasIndex("ProductionStageId");

                    b.HasIndex("WorkerId", "ProductionStageId", "Date");

                    b.ToTable("DailyProductions");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.HourlyWorkLog", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("Date")
                        .HasColumnType("TEXT");

                    b.Property<int>("EndHour24")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Notes")
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.Property<decimal>("WorkdaysCredited")
                        .HasColumnType("decimal(5,2)");

                    b.Property<int>("WorkerId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("Date");

                    b.HasIndex("WorkerId", "Date")
                        .IsUnique();

                    b.ToTable("HourlyWorkLogs");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Penalty", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("Date")
                        .HasColumnType("TEXT");

                    b.Property<int>("Deduction")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Notes")
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.Property<string>("Reason")
                        .IsRequired()
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.Property<int>("Source")
                        .HasColumnType("INTEGER");

                    b.Property<int>("WorkerId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("Date");

                    b.HasIndex("WorkerId", "Date");

                    b.ToTable("Penalties");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Product", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("TEXT");

                    b.Property<byte[]>("ImageData")
                        .HasColumnType("BLOB");

                    b.Property<bool>("IsActive")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("TEXT");

                    b.Property<string>("ProductCode")
                        .HasMaxLength(30)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("Name");

                    b.ToTable("Products");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.ProductionBatch", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime?>("CompletedDate")
                        .HasColumnType("TEXT");

                    b.Property<bool>("IsOpeningBalance")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("LastCompletedStageId")
                        .HasColumnType("INTEGER");

                    b.Property<string>("Notes")
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.Property<int>("ProductId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("Quantity")
                        .HasColumnType("INTEGER");

                    b.Property<int?>("SplitFromBatchId")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("StartedDate")
                        .HasColumnType("TEXT");

                    b.Property<int>("Status")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("CompletedDate");

                    b.HasIndex("LastCompletedStageId");

                    b.HasIndex("SplitFromBatchId");

                    b.HasIndex("StartedDate");

                    b.HasIndex("ProductId", "Status");

                    b.ToTable("ProductionBatches");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.ProductionDayClosure", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("CarriedBatchCount")
                        .HasColumnType("INTEGER");

                    b.Property<int>("CarriedPieces")
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("ClosedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("Date")
                        .HasColumnType("TEXT");

                    b.Property<string>("Notes")
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("Date")
                        .IsUnique();

                    b.ToTable("ProductionDayClosures");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.ProductionStage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<bool>("IsActive")
                        .HasColumnType("INTEGER");

                    b.Property<int>("PiecesPerWorkday")
                        .HasColumnType("INTEGER");

                    b.Property<int>("ProductId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("SortOrder")
                        .HasColumnType("INTEGER");

                    b.Property<string>("StageName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("ProductId");

                    b.ToTable("ProductionStages");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.WageAdjustment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<decimal>("AmountEgp")
                        .HasColumnType("decimal(10,2)");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<DateTime>("Date")
                        .HasColumnType("TEXT");

                    b.Property<string>("Note")
                        .HasMaxLength(300)
                        .HasColumnType("TEXT");

                    b.Property<int>("Type")
                        .HasColumnType("INTEGER");

                    b.Property<int>("WorkerId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("Date");

                    b.HasIndex("WorkerId", "Date");

                    b.ToTable("WageAdjustments");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Worker", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("TEXT");

                    b.Property<decimal>("DailyWageEgp")
                        .HasColumnType("decimal(10,2)");

                    b.Property<string>("EmployeeCode")
                        .HasMaxLength(30)
                        .HasColumnType("TEXT");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("TEXT");

                    b.Property<DateTime?>("HireDate")
                        .HasColumnType("TEXT");

                    b.Property<int?>("HourlyRole")
                        .HasColumnType("INTEGER");

                    b.Property<bool>("IsActive")
                        .HasColumnType("INTEGER");

                    b.Property<string>("PhoneNumber")
                        .HasMaxLength(20)
                        .HasColumnType("TEXT");

                    b.Property<string>("SkillsNotes")
                        .HasMaxLength(1000)
                        .HasColumnType("TEXT");

                    b.HasKey("Id");

                    b.HasIndex("FullName");

                    b.ToTable("Workers");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.WorkerSkill", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("INTEGER");

                    b.Property<int>("Level")
                        .HasColumnType("INTEGER");

                    b.Property<int>("ProductionStageId")
                        .HasColumnType("INTEGER");

                    b.Property<int>("WorkerId")
                        .HasColumnType("INTEGER");

                    b.HasKey("Id");

                    b.HasIndex("ProductionStageId");

                    b.HasIndex("WorkerId", "ProductionStageId")
                        .IsUnique();

                    b.ToTable("WorkerSkills");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Attendance", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.Worker", "Worker")
                        .WithMany("AttendanceRecords")
                        .HasForeignKey("WorkerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Worker");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.DailyProduction", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.ProductionBatch", "ProductionBatch")
                        .WithMany("Productions")
                        .HasForeignKey("ProductionBatchId")
                        .OnDelete(DeleteBehavior.SetNull);

                    b.HasOne("WorkforceManager.Core.Models.ProductionStage", "ProductionStage")
                        .WithMany("ProductionRecords")
                        .HasForeignKey("ProductionStageId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("WorkforceManager.Core.Models.Worker", "Worker")
                        .WithMany("ProductionRecords")
                        .HasForeignKey("WorkerId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.Navigation("ProductionBatch");

                    b.Navigation("ProductionStage");

                    b.Navigation("Worker");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.HourlyWorkLog", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.Worker", "Worker")
                        .WithMany("HourlyWorkLogs")
                        .HasForeignKey("WorkerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Worker");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Penalty", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.Worker", "Worker")
                        .WithMany("Penalties")
                        .HasForeignKey("WorkerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Worker");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.ProductionBatch", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.ProductionStage", "LastCompletedStage")
                        .WithMany()
                        .HasForeignKey("LastCompletedStageId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.HasOne("WorkforceManager.Core.Models.Product", "Product")
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired();

                    b.HasOne("WorkforceManager.Core.Models.ProductionBatch", "SplitFromBatch")
                        .WithMany()
                        .HasForeignKey("SplitFromBatchId")
                        .OnDelete(DeleteBehavior.Restrict);

                    b.Navigation("LastCompletedStage");

                    b.Navigation("Product");

                    b.Navigation("SplitFromBatch");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.ProductionStage", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.Product", "Product")
                        .WithMany("Stages")
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Product");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.WageAdjustment", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.Worker", "Worker")
                        .WithMany("WageAdjustments")
                        .HasForeignKey("WorkerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Worker");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.WorkerSkill", b =>
                {
                    b.HasOne("WorkforceManager.Core.Models.ProductionStage", "ProductionStage")
                        .WithMany("QualifiedWorkers")
                        .HasForeignKey("ProductionStageId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("WorkforceManager.Core.Models.Worker", "Worker")
                        .WithMany("Skills")
                        .HasForeignKey("WorkerId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ProductionStage");

                    b.Navigation("Worker");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Product", b =>
                {
                    b.Navigation("Stages");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.ProductionBatch", b =>
                {
                    b.Navigation("Productions");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.ProductionStage", b =>
                {
                    b.Navigation("ProductionRecords");

                    b.Navigation("QualifiedWorkers");
                });

            modelBuilder.Entity("WorkforceManager.Core.Models.Worker", b =>
                {
                    b.Navigation("AttendanceRecords");

                    b.Navigation("HourlyWorkLogs");

                    b.Navigation("Penalties");

                    b.Navigation("ProductionRecords");

                    b.Navigation("Skills");

                    b.Navigation("WageAdjustments");
                });
#pragma warning restore 612, 618
        }
    }
}
