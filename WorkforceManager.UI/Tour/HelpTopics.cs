    ...
    public static class HelpTopics
    {
        public static IReadOnlyList<HelpTopic> Topics { get; } = new List<HelpTopic>
        {
            new()
            {
                ...
                Icon = "AccountGroup",
                Description =
                    "قايمة كل العمال ببطاقات: حالة، مهارات. بحث فوري بالاسم، وزرار \"فلاتر " +
                ...
                TourSteps = new List<AppTourStep>
                {
                    ...
                    new()
                    {
                        Title = "عامل جديد",
                        Description =
                            "إضافة عامل جديد ببياناته الأساسية. اكتب الاسم وسعر اليومية واحفظ " +
                ...
                TourSteps = new List<AppTourStep>
                {
                    new()
                    {
                        ...
                        Description =
                            ...
                            "أول ما اليوم ده ييجي. تقدر تعدّل أي خطة موجودة بنفس الفورم ده كمان — لو بدّلت خطة وانت لسه " +
                            "كاتب حاجة في فورم تاني، البرنامج بيسألك تأكيد قبل ما يضيّع اللي كتبته.",
                    ...
                    new()
                    {
                        ...
                        Description =
                            ...
                            "أو \"رجّعها نشطة\" لو اتعلّمت منجزة غلط (فتحت شاشتها بس ماسجّلتش إنتاج فعلي فيها). شارة " +
                            "\"متأخر\" أو \"النهارده\" على الكارت بتوريك فورًا أنهي خطط مستحقة من غير ما تفتح كل واحدة. " +
                            "لو المنتج أو مرحلة في الخطة اتوقفت بعد ما كتبتها، بيظهر تحذير واضح على الكارت و\"ابدأها " +
                            "دلوقتي\" بيتقفل لحد ما تظبط المشكلة.",
                    ...
                    new()
                    {
                        Title = "تبويب \"منجزة\"",
                        Description =
                            "خطة بتتنقل هنا **بس** بعد أول تسجيل إنتاج حقيقي عليها — مش لمجرد ما تفتح شاشتها من " +
                            "التذكير. كروت القسم ده للمراجعة بس، ولو خطة اتعلّمت منجزة بالغلط (فتحتها بس ماسجّلتش " +
                            "فيها حاجة فعليًا) تقدر ترجّعها نشطة من هنا.",
                        TargetElementName = "MemoryListsPanel",
                        Screen = TourScreen.Memory
                    },
                    new()
                    {
                        Title = "تذكير بيظهر لما تفتح البرنامج",
                        Description =
                            "أي خطة نشطة موعدها النهارده أو فات بتظهرلك في نافذة تذكير أول ما تسجّل دخول — واحدة في " +
                            "المرة (مش كلهم مكدّسين فوق بعض)، وفيها \"ابدأها دلوقتي\" أو \"أجّل\". لو دوست \"ابدأها\" " +
                            "على أول واحدة، الباقي بيستناك في تسجيل الدخول اللي بعده — مش هيضيع، وهتلاقي رسالة بتقولك " +
                            "كام خطة لسه مستنية.",
                        TargetElementName = "MemoryListsPanel",
                        Screen = TourScreen.Memory
                    }
            ...
            new()
            {
                ...
                Icon = "ChartBar",
                Description =
                    "لوحة متابعة حيّة بتتقرا على الشاشة، مش للتصدير أو الطباعة (شوف \"التقارير\" لده). " +
                    "3 تبويبات: \"الإنتاج\" (أرقام اليوم/الأسبوع/الشهر لكل منتج)، \"إنتاج المنتجات\" " +
                    "(رسم بياني بمرور الوقت)، و\"متوسط إنتاج العمال\" (كل عامل مقارن بمتوسطه هو الشخصي).",
                TourSteps = new List<AppTourStep>
                {
                    new()
                    {
                    ...
                    new()
                    {
                        Title = "عمال إنتاجهم قلّ",
                        Description =
                            "كارت بيظهر بس لو فيه عمال إنتاجهم النهارده قلّ بشكل ملحوظ عن متوسط أيام شغلهم الأخيرة — " +
                            "مجمّعين هنا في مكان واحد بدل ما تدوّر عليهم واحد واحد في شاشة العمال.",
                        TargetElementName = "DecliningWorkersCard",
                        Screen = TourScreen.Evaluation
                    },
                    new()
                    {
                    ...
                    new()
                    {
                        Title = "المقارنة والمتوسط",
                        Description =
                            "تحت الرسم البياني: سهم ▲/▼ بيقارن الفترة المختارة بنفس طول الفترة اللي قبلها فورًا (زيادة " +
                            "أو نقصان بالنسبة)، وخط رفيع بيوري متوسط الإنتاج محسوب بس من الفترات اللي فيها شغل فعلي " +
                            "(مش بيتأثّر بالجمعة أو أيام الإجازات).",
                        TargetElementName = "ChartTrendSummaryCard",
                        Screen = TourScreen.Evaluation
                    },
                    new()
                    {
                        Title = "متوسط إنتاج العمال",
                        Description =
                            "تبويب مستقل: متوسط إنتاج كل عامل على آخر 7 أيام شغل فعلية **له هو بس** (مش مقارنة بزمايله). " +
                            "زرار الترتيب بيقلب بين الأعلى والأقل إنتاجًا، وفلتر \"اللي قلّ بس\" بيخفي أي عامل إنتاجه " +
                            "النهارده عادي ويسيب بس اللي قلّ عن معدّله. دوس على أي عامل يتمدد ويوريك أيامه الأخيرة " +
                            "بالتفصيل.",
                        TargetElementName = "WorkerAveragesToolbar",
                        Screen = TourScreen.Evaluation
                    }
                }
            },
            new()
            {
                ...
                Description =
                    "بناء تقرير مخصّص يتصدّر Excel: اختار الموضوع (إنتاج/حضور/هالك/مهارات/أجور/جزاءات/" +
                    "سلف وحوافز/الحسابات الإدارية)، طريقة التجميع، الفترة، وأي فلاتر. الأعمدة نفسها " +
                    "تقدر تظهرها أو تخفيها أو تعيد ترتيبها، والمعاينة بتتحدّث فورًا مع كل اختيار. " +
                    "الإعدادات اللي بتستخدمها كتير تقدر تحفظها كقالب جاهز — وتقرير الأجور بالذات فيه " +
                    "طريقة تانية للطباعة، قسايم أجر جاهزة للتقصيص.",
                TourSteps = new List<AppTourStep>
                {
                    new()
                    {
                    ...
                    new()
                    {
                        Title = "الموضوع",
                        Description =
                            "أول وأهم اختيار — إنتاج، حضور، أجور، هالك، مهارات، جزاءات، سلف وحوافز، أو الحسابات " +
                            "الإدارية. كل حاجة تانية في الشاشة (التجميع المتاح، الفلاتر، الأعمدة) بتتغيّر حسبه.",
                    ...
                    new()
                    {
                        Title = "تجميع متقدم",
                        Description =
                            "لتقرير الإنتاج بس: مستوى تجميع إضافي أو اتنين فوق الأساسي — \"بالمنتج ثم العامل ثم " +
                            "المرحلة\" مثلًا، لسؤال أدقّ زي \"مين اشتغل على إيه بالظبط\". مطويّة افتراضيًا لأنها " +
                            "حالة خاصة، مش التقرير العادي.",
                        TargetElementName = "AdvancedGroupingCheck",
                        Screen = TourScreen.Reports
                    },
                    new()
                    {
                        Title = "فلاتر",
                        Description =
                            "حسب الموضوع: عمال محدّدين (اختيار متعدد)، منتجات محدّدة، مرحلة بعينها، أو نوع العامل " +
                            "(بالقطعة/بالساعة). كلها بتتراكب مع بعض، وبادج على الزرار بيقولّك كام فلتر شغال دلوقتي.",
                        TargetElementName = "FilterToggle",
                        Screen = TourScreen.Reports
                    },
                    new()
                    {
                        Title = "الأعمدة",
                        Description =
                            "لكل عمود: إظهار/إخفاء، تغيير اسمه (من غير ما يأثّر على القالب المحفوظ)، ضمّه لصف " +
                            "الإجمالي تحت الجدول لو رقم قابل للجمع، وترتيبه بالأسهم فوق/تحت. \"رجّع الأعمدة زي ما " +
                            "كانت\" بيصفّر كل التعديلات دي مرة واحدة.",
                        TargetElementName = "ColumnToggle",
                        Screen = TourScreen.Reports
                    },
                    new()
                    {
                        Title = "الترتيب والعرض",
                        Description =
                            "رتّب الجدول بعمود معيّن (تصاعدي/تنازلي)، اقصره على أعلى/أقل N صف (5/10/20/50)، ضيف صف " +
                            "إجمالي تحت الجدول، أو فعّل عمود مقارنة بالفترة اللي قبلها مباشرة لكل رقم.",
                        TargetElementName = "DisplayToggle",
                        Screen = TourScreen.Reports
                    },
                    new()
                    {
                        ...
                        Description =
                            "قبل \"تصدير Excel\": اختار شيت تفاصيل (كل سجل على حدة، للتحليل والفرز) أو شيت لكل " +
                            "مجموعة (منفصلين، جاهزين للطباعة والتوزيع). اسم المصنع والشعار بييجوا من الإعدادات " +
                            "تلقائي.",
                        TargetElementName = "ExportToggle",
                        Screen = TourScreen.Reports
                    },
                    new()
                    {
                    ...
                    new()
                    {
                        Title = "قسايم الأجر",
                        Description =
                            "بيظهر بس لما الموضوع يكون \"أجور\". بيطبّع ورقة A4 بالعرض فيها 4 قسايم أجر للطباعة " +
                            "والتقصيص، لعمال محدّدين لو فلترت عليهم — نفس أرقام تقرير الأجور بالظبط، لأنه بيقرا من " +
                            "نفس مصدر الحساب.",
                        TargetElementName = "ExportPayslipStripsButton",
                        Screen = TourScreen.Reports
                    },
                    new()
                    {
                        Title = "بنود القسيمة وفورمات محفوظة",
                        Description =
                            "علّم بس على البنود اللي عايزها تظهر في القسيمة (سعر اليومية، تفصيل المراحل، الخصومات...) " +
                            "— اختيارك بيتحفظ لوحده ويتستخدم في كل تصدير جاي. تقدر كمان تحفظ مجموعة بنود باسم " +
                            "(\"فورمات\") وتبدّل بينها بضغطة، بدل ما تعلّم/تشيل نفس البنود كل مرة.",
            ...
            new()
            {
                ...
                TourSteps = new List<AppTourStep>
                {
                    ...
                    new()
                    {
                        Title = "أسباب الهالك",
                        Description =
                            "قايمة أسباب جاهزة تظهر وقت تسجيل هالك في التسجيل اليومي (\"عيب في القماش\"، \"كسر\"...) — " +
                            "بدل ما كل واحد يكتب السبب بصياغته الخاصة، فتقرير الهالك يقدر يجمّع الأسباب المتشابهة. " +
                            "السبب مايتمسحش أبدًا، بس تقدر توقفه فيختفي من الاختيار الجديد من غير ما يأثّر على سجلات " +
                            "قديمة استخدمته.",
                    ...
                    new()
                    {
                        Title = "هوية التقارير",
                        Description =
                            "اسم المصنع والقسم وشعار التقارير — بيتطبعوا فوق كل ملف Excel بيتصدّر من شاشة التقارير، " +
                            "والاسم والقسم كمان بيظهروا في رأس القايمة الجانبية وعنوان النافذة. مفيد خصوصًا لو نفس " +
                            "المصنع شغّال بأكتر من نسخة لأقسام مختلفة.",
                    ...
                    new()
                    {
                        Title = "شعار البرنامج",
                        Description =
                            "منفصل تمامًا عن شعار التقارير فوق — ده بيحل محل شعار WMS جوّه البرنامج نفسه (القايمة " +
                            "الجانبية، شاشة تسجيل الدخول، أيقونة النافذة)، مش هيدر ورقة مطبوعة. صورة مختلفة غالبًا عن " +
                            "شعار التقارير.",
                    ...
                    {
                        Title = "الوضع الليلي",
                        Description = "ألوان غامقة أريح للعين في إضاءة واطية — بيتفعّل بعد إعادة تشغيل البرنامج، مش فورًا.",
                        TargetElementName = "DarkModeCard",
                        Screen = TourScreen.Settings
                    },
                    new()
                    {
                        Title = "مدة الاحتفاظ بسجل العمليات",
                        Description =
                            "مدتين منفصلتين: أحداث الحذف (مسح يوم/سجل/عامل/منتج/مرحلة) ومدة تانية لأحداث الفلوس (أجر/" +
                            "جزاء/سلفة) — الفلوس عادة محتاجة مدة أطول. صفر يعني \"من غير مسح خالص\". الإعداد ده هو " +
                            "اللي شاشة \"سجل العمليات\" بتحيل عليه في السطر تحت قايمتها.",
                    ...
                    new()
                    {
                        ...
                        Description =
                            "آخر سطر في الشاشة: رقم الإصدار الحالي، اسم المصمّم، تاريخ أول وآخر إصدار، ومكان مجلد " +
                            "بياناتك بالظبط على الجهاز — أول حاجة هتحتاجها في أي مكالمة دعم فني.",
