...
namespace WorkforceManager.UI.ViewModels
{
    ...
    public partial class FlowSessionViewModel : ObservableObject
    {
        ...
        ///
        /// **الشاشة بتعيش طول الجلسة** (Scoped على نطاق تسجيل الدخول)،
        /// فسيبان القيمة دي معناها إن جلسة عادية بعد كده تلاقي ترتيب خطة
        /// قديمة وتتحقق بيه. عشان كده بتتصفّر في
        /// <see cref="OnSelectedProductChanged"/>، وبتموت مع نطاق الجلسة
        /// عند تسجيل الخروج — ومحدش بيعتمد على إنها هتتنسى لوحدها.
        /// </summary>
        ...
        // كارت "الشغل الواقف" (PendingStages/HasPendingWork/ResumePending) اتشال من
        // الشاشة — أي نطاق ناقص بقى بيتحول رصيد أولي تلقائيًا (ProductionFlowService)،
        // ومكانه الوحيد دلوقتي تبويب "الرصيد الأولي". PendingWorkService نفسها
        // لسه موجودة ومستخدمة في HistoricalPendingMigrationService والاختبارات.
